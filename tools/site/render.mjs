...
function sections(item, ui, icons) {
...
export function releaseBody(item, ui, icons) {
  const meta=ui.musicDetails.filter(f=>item[f.key]).map(f=>`<p><span class="music-release-detail__label">${escape(f.label)}</span><span>${escape(item[f.key])}</span></p>`).join('');
...
${item.tracks?.length ? `<section><h2>${escape(ui.musicTrackListLabel)}</h2><ol class="release-document__tracks">${item.tracks.map(t=>`<li id="track-${escape(t.id)}"><details><summary>${escape(t.title)} <span>${escape(t.duration)}</span>${disclosureIcon(ui,icons)}</summary><p>${escape(t.format || item.format || '')}</p>${t.isrc?`<p>ISRC: ${escape(t.isrc)}</p>`:''}${sections(t,ui,icons)}${links(t.links,icons)}</details></li>`).join('')}</ol></section>`:''}
</article>`;
}
